...
public class StockMarketApplication {
    ...
    public List<Integer> getCustomersWhoSoldManyStocks(int numDifferentStocksSold)
    {
        if(numDifferentStocksSold <= 0) {
            System.out.println("Make sure numDifferentStocks is greater than zero.");
            System.exit(-1);
        }
        List<Integer> result = new ArrayList<Integer>();
        // your code here
        try {
            PreparedStatement st = connection.prepareStatement("select distinct t.sellerID from Trades t where t.sellerID in (select distinct t1.sellerID from Trades AS t1 group by t1.sellerID having count(*) > ?)");
            st.setInt(1, numDifferentStocksSold);
            ResultSet rs = st.executeQuery();

            //System.out.print("\n---------CustomerIDs of those who sold " + numDifferentStocksSold + " stocks--------- \n");
            while (rs.next())
            {
    ...
    public int updateQuotesForBrexit(String theExchangeID)
    {
        ...
        int result = 0;
        try {
            PreparedStatement st = connection.prepareStatement("UPDATE Quotes SET price = price * 0.87 WHERE exchangeID = ?");
    ...
    public int rewardBestBuyers (int theSellerID, int theCount)
    {
        ...
        // your code here
        try {
            //CallableStatement st = connection.prepareCall("{ ? = CALL rewardBuyersFunction(?,?)}");
            //st.registerOutParameter(1, integer);
            PreparedStatement st = connection.prepareStatement("SELECT rewardBuyersFunction(?,?)");
        ...
        //GET DIAGNOSTICS storedFunctionResult = ROW_COUNT;
        // end of your code
